namespace Loupedeck.KG_RStudioPlugin
{
    using System;
    public class Previous_Terminal : PluginDynamicCommand
    {
        public Previous_Terminal()
        : base(displayName: "Previous Terminal", description: "Previous Terminal", groupName: "Terminal")
        {
        }
        protected override BitmapImage GetCommandImage(String actionParameter, PluginImageSize imageSize)
        {
            using (var bitmapBuilder = new BitmapBuilder(imageSize))

            {

                bitmapBuilder.SetBackgroundImage(EmbeddedResources.ReadImage(EmbeddedResources.FindFile("Terminal.png")));
                bitmapBuilder.DrawText("Previous Terminal", color: new BitmapColor(37,26,25));
                return bitmapBuilder.ToImage();

            }
        }
        protected override void RunCommand(String actionParameter)
        {
            this.Plugin.ClientApplication.SendKeyboardShortcut(F11, ModifierKey.Alt | ModifierKey.Shift);
        }
    }
}
